...
export function GoalPage(){
 ...
 useEffect(()=>{if(!id||!goals.data)return;setGoal(goals.data.find(x=>x.id===id)||null);Promise.all([getInitiatives(id),getIndicators(id)]).then(async([a,b])=>{setInitiatives(a);setIndicators(b);const hs:HistoryMap={},av:AvgMap={};await Promise.all(b.map(async k=>{hs[k.id]=await getIndicatorHistory(k.id);if(k.year!=='—'){const gr=await getStudentGroups(k.id,k.year);const avg=gr.find((r:any)=>String(r.category||'').trim().toLowerCase()==='statewide average'||String(r.student_group||'').trim().toLowerCase()==='statewide average'||String(r.category||'').trim().toLowerCase()==='statewide'||String(r.student_group||'').trim().toLowerCase()==='statewide');if(avg)av[k.id]=String(avg.value_3_display??avg.value_3_numeric??'—')}}));setHistories(hs);setStateAvgs(av)}).catch(e=>setError(e.message))},[id,goals.data]);
